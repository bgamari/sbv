...
p1 :: Symbolic SBool
p1 = do constrain $ \x -> thingCompare x x
        constrain $ \k1 k2 -> k1 ./= thingMerge k1 k2
...
testQuery rf = do r <- runSMTWith defaultSMTCfg{verbose=True, redirectVerbose=Just rf} t
 ...
 where t = do vp <- free "p"
              ...
              query $ do let oR, aND :: SB  -> SB -> SB
                         ...
                         constrain $ \p q r -> (p `oR` q) `aND` (p `oR` r) .== p `oR` (q `aND` r)
                         constrain $ \p q   -> nOT (p `oR` q) .== nOT p `aND` nOT q
                         constrain $ \p     -> nOT (nOT p) .== p
                         checkSat
