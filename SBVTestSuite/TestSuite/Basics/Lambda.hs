...
tests =
  testGroup "Basics.Lambda" $ [
      ...
      , goldenCapturedIO "lambda36" $ record $ \st -> constraintStr st sTrue
      , goldenCapturedIO "lambda37" $ record $ \st -> constraintStr st sNot
      , goldenCapturedIO "lambda38" $ record $ \st -> constraintStr st (\x y -> x .== (0 :: SInteger) .|| y)
